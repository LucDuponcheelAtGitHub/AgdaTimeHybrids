...
underlying mathematical abstraction of his Time Hybrids book.
...
Frankly, my mathematical abstraction may not be fully in sync with Fred's mathematical
...
- **Profunctors**: The inter-action between the virtual universe and the material universe is
  ...
  pre-places is modeled as a heteromorphism, upon which material pre-thing collection
  transitions act contravariantly at left and virtual pre-place transitions act covariantly at
  right, respectively.

Note that I used the word *inter-action* to emphasize that both pre-thing collection transitions
and pre-place transitions act upon their cross-category connection.
...
Frankly, also motivated by personal preferences ... .
...
- **Computational Univalence**: The *Univalence Principle* states that type equivalence
  ...
  to  path equalities without needing unproven `postulate`s.

Perhaps, more surprisingly, also the following is striking.

- **Harmonizing Paths and Transitions**: Category theory provides transitions **at** which
  state evolves. Homotopy Type Theory provides paths **at** which equality and structure are
  `transport`ed. In our formalization, these two concepts work in complete harmony. For
  example, path equalities between types are transported to path equalities between
  ...
  *paths at*, emphasising that I consider *continuos* paths to be discrete entities. After all, 
  under the hood, they correspond to structural induction.
...
turned out to be the natural abstraction to *formally* separate virtual universe related concepts
from material universe related concepts.

It is perfectly possible to go for other abstractions, but, as far as I know, they rely on
pre-places to belong to both the virtual universe and the material universe. They are, in my
opinion, sub-optimal abstractions.
...
## The `Reality` Specification (version `01`)
...
- `Cᵥ`: the *virtual universe category* with *virtual pre-places*. The virtual universe is
  dynamic using *virtual universe transitions*. Virtual pre-places are dynamic using 
  *intra-functorially* corresponding *virtual pre-place transitions*.

- `Cₘ`: the *material universe category* with *material pre-thing collections*. Material
  pre-thing collections are dynamic using *inter-functorially* corresponding
  *material pre-thing collection transitions*.

and a parameter

- `PFₘᵥ`: the *material-virtual profunctor*, formalizing the inter-action between the virtual
  universe and the material universe.

and a parameter:

- `CFₘ`: the *material collection functor*, formalizing to-collection mappings in the
  material universe.
...
- `Uᵥ`: formalizing a unique designated virtual universe object that you can think of as the
  *state* of the virtual universe or, maybe more spectacular, as the *time* of the virtual
  universe. Virtual universe transitions make the virtual universe dynamic. Viewing virtual
...
Pre-thing collections can then be seen as non-existing *composite pre-things* brewing what,
eventually, may become *composite existing things*.

Note that there are some *implicit `Level` parameters* involved. They are necessary to avoid
well known paradoxes that would occur when formalizing theories in a naive way. 
...
The code below defines some, notationally convenient, renamings and synonyms.
...
- `preThingsFunctorᵥₘ` formalizes mapping virtual *universe* transitions to material
*pre-thing collection* transitions, making pre-thing collections dynamic. Note that two
...
- `PrePlaceᵥ` is the `ob` level part of `prePlaceFunctorᵥ` at `Uᵥ`.

- `PreThingsᵥₘ` is the `ob` level part of `preThingsFunctorᵥₘ` at `Uᵥ`.
...
- `PreThingsᵥₘ≡PreThingsₘ` declares the equivalence of the previous subsection.
...
We are ready now to characterise *movement of pre-thing collections* in a dynamic universe
setting (where pre-places are dynamic as well). 

### `MovementFunctorᵥ`

- `MovementFunctorᵥ` specifies a virtual universe category functor `Fᵥ` that requires 
   equality of `PrePlaceᵥ` with `Fᵥ ⟅ Uᵥ ⟆` at `ob` level.
...
has been declared as an implicit `Level` typed parameter of `Reality01`.

`isMovementAtUniverseTransitionᵥ` characterises movement functorially as a `MovementFunctorᵥ`
so that `movementTransitionᵥ`, defined in terms of it, equals 

- the result of the left acting of pre-thing collection transitions upon pre-thing collection
  pre-places
  
and 

- the result of the right acting of pre-place transitions upon pre-thing collection pre-places

by 

- letting the movement transition right act upon the former result.
...
- movement transitions in terms of equality of `PrePlaceᵥ` and `Fᵥ⟅Uᵥ⟆` at the `ob` level.

### `isImmobileAtUniverseTransitionᵥ`

Characterising *immobility* is now also simple by using a *constant* virtual pre-place functor
`Constant Cᵥ Cᵥ PrePlaceᵥ` that *definitionally* (using `refl` evidence), deals with
