...
The operations `_ₗ∘_` and `_∙ᵣ_` below are the left `Homₘ` and right `Homᵥ` actions on `Hetₘᵥ`
respectively.
...
Note that `lhs ∙ᵣ movementTransitionᵥ ≡ rhs`, the `Property` involved, is an equality.

Note how naturally HoTT deals with defining

- pre-thing collection transition equality in terms of transporting pre-thing collection
  equality along an interval `I` from `i0` to `i1`.

and 

- pre-place transition equality in terms of transporting pre-place equality along an interval
  `I` from `i0` to `i1`.

Note that, for `preThingsTransitionₘ`, `PreThingsᵥₘ≡PreThingsₘ i1` equals `PreThingsₘ` and
for `movementTransitionᵥ`, `Fᵥ⟅Uᵥ⟆≡PrePlaceᵥ i1` equals `PrePlaceᵥ` respectively.
...
We wrote a formal specification `record Reality01` with 
...
two categories, `Cᵥ` (virtual universe) and `Cₘ` (material universe)

- `(PFₘᵥ : Profunctor⊶ o h Cₘ Cᵥ)`

a profunctor `PFₘᵥ` relating `Cᵥ` and `Cₘ`

- `(CFₘ : Functor Cₘ Cₘ)`

a collection endofunctor `CFₘ` in `Cₘ`

- `(Uᵥ : Category.ob Cᵥ)`
- `(Pₘ : Category.ob Cₘ)`

`Uᵥ` (state/time), a dedicated `Cᵥ` object and `Pₘ` (potentials), dedicated `Cₘ` objects. 
...
`prePlaceFunctorᵥ` lifting the dynamic nature of `Cᵥ` to `PrePlaceᵥ`
(pre-places), see below, and
`preThingsFunctorᵥₘ` lifting the dynamic nature of `Cᵥ` to `PreThingsᵥₘ`
(pre-thing collections), see below.

Note that this lifting happens with pre-thing collections.

- `PreThingₘ : obₘ`

`PreThingₘ` (pre-things)

- `PreThingsᵥₘ≡PreThingsₘ : PreThingsᵥₘ ≡ PreThingsₘ`

An equality law stating that pre-thing collections are collections of pre-things, see below
for the definition of `PreThingsₘ`.

- `PreThingsₘ⊶PrePlaceᵥ : Hetₘᵥ[ PreThingsₘ , PrePlaceᵥ ]`  

A heteromorphism (pre-thing collection pre-place).

#### definitions

- `PrePlaceᵥ : obᵥ` as `prePlaceFunctorᵥ ⟅ Uᵥ ⟆`

See above for the declaration of `prePlaceFunctorᵥ`.

- `PreThingsᵥₘ` as `preThingsFunctorᵥₘ ⟅ Uᵥ ⟆`

See above for the declaration of `preThingsFunctorᵥₘ`.

- `PreThingsₘ` as `CFₘ ⟅ PreThingₘ ⟆`

See above for the declaration of `PreThingsᵥₘ≡PreThingsₘ` where it is used.

#### a nested formal specification `record PrePlaceFunctorᵥ`

`record PrePlaceFunctorᵥ` has `field` declarations

- `Fᵥ : Functor Cᵥ Cᵥ`

An endofunctor `Fᵥ` in `Cᵥ`.

- `Fᵥ⟅Uᵥ⟆≡PrePlaceᵥ : Fᵥ ⟅ Uᵥ ⟆ ≡ PrePlaceᵥ`

An equality law stating that object `Fᵥ ⟅ Uᵥ ⟆` equals object `PrePlaceᵥ`.

#### Movement in terms of a `Property`

In terms of all declarations and definitions above we then defined movement in terms of a
`Property`

- `isMovementAtUniverseTransitionᵥ : PrePlaceFunctorᵥ → Homᵥ[ Uᵥ , Uᵥ ] → Property`

Defining movement as a virtual pre-place functor correcting two pre-thing collection pre-places
to be equal by *right-acting* on one of them.

#### Immobility as no movement

- `isImmobileAtUniverseTransitionᵥ : Homᵥ[ Uᵥ , Uᵥ ] → Property`

Defined in terms of `isMovementAtUniverseTransitionᵥ` above.
...
### `reality01` `field` and `terminalₘ` `field` declarations
...
The dichotomy respectation property is defined below as `PreInteractionPreservation`.
...
Note that this time, the `Property` involved, is an implication.

### Summary

We wrote a formal specification `record Reality02` with 
...
`reality01` can be used to inherit `record Reality01` features by delegation, and
`terminalₘ` category `Cₘ` (material universe) is now a category with terminal objects.

- `isGlobalPreInteractionₘ : Homₘ[ 1ₘ , PreThingsₘ ] → Property`

A property distinguishing pre-thing collection of interacting pre-things from other pre-thing
collections. See below for the declarations of `1ₘ`.

#### definitions

- `1ₘ : obₘ` as `terminalOb Cₘ terminalₘ`

See above for the declaration of `terminalₘ`.

- `PreInteractionₘ : obₘ` as `PreThingsₘ`

See above how to distinguish `PreInteractionₘ` objects, pre-interactions, from `PreThingsₘ`
objects, pre-thing collections.

- f ⋙ₘ g = g ∘ₘ f

A left-to-right version of right-to-left material universe homomorphism composition.

- `preThingsTransitionᵥₘToPreThingsTransitionₘAt`

Mapping virtual universe transitions to material pre-thing collection transitions.

#### `Property`

In terms of all declarations and definitions above we then defined a `Property`

- `PreInteractionPreservation`

An implication stating that being a pre-interaction is preserved by pre-thing collection
transitions.
...
### `reality02` `field` and `nothingₘ` `field instance` declaration
...
`nothingₘ` is a `field instance` declaration of `Zero`.
...
```
### Corresponding `Homₘ[ PreThingsₘ , PreThingsₘ ]` definition

`toNothingTransitionₘ` defines the material transition of pre-thing collections to nothing,
...
### `NoEscapeFromNothingₘ` and it's proof `noEscapeFromNothingProofₘ`
...
Note that, this time, the `Property` involved is an equality.
...
- `reality02 : Reality02 Cᵥ Cₘ PFₘᵥ CFₘ Uᵥ Pₘ`
- `instance nothingₘ : Zero Cₘ CFₘ`

`reality02` can be used to inherit `record Reality02` features by delegation, and
`nothingₘ` category `Cₘ` (material universe) is now a category with a *zero* collection
endofunctor `instance`.

#### definition

- `toNothingTransitionₘ : Homₘ[ PreThingsₘ , PreThingsₘ ]` as `N-ob (ζ ⦃ nothingₘ ⦄) PreThingₘ`

See above for the declaration of `nothingₘ`.
...
The `Property` as just a specialization of a the `absorption` law of `Zero`, so its proof is
trivial.

## The `Reality04` specification

The specification `record Reality04` builds upon the specification `record Reality03` .

Our next law is an *equivalence*.

Again we leverage standard libraries to keep `field` declarations and corresponding definitions
`Cubical Agda` idiomatic using

- Equivalence (`≃`) from `Agda.Builtin.Cubical.Equiv`.

```agda
open import Agda.Builtin.Cubical.Equiv
```

It turns out our next law is related to *monads*.

Again we leverage standard libraries to keep `field` declarations and corresponding definitions
`Cubical Agda` idiomatic using

- Monads from `Cubical.Categories.Monad.Base`.
...
### `reality03` `field` and `isMonad` `field instance` declaration

`reality03` is a `field` declaration that can be used to access the `field` declarations and
definitions of `Reality02` (in `Reality04` and later versions) by opening it `public` using
...
Not all pre-thing collections are pre-thing singletons. We declare
`field` `isGlobalPreThingSingletonₘ`, a property to make the distinction. It is formulated in
terms of global values (elements), a common name for values (elements) of type `Homₘ[ 1ₘ , Z ]`
for some `Z` (in this case `PreThingsₘ`).

We also declare `field` `preThingsAsPreThingₘ` a structural requirement stating that pre-thing
collections can morph to pre-things in order to be used as pre-things.
...
### Corresponding `Homₘ[ PreThingₘ , PreThingSingletonₘ ]` definition
...
## Pre-thing singleton equivalence

We are ready to formulate an equivalence between the structural .
